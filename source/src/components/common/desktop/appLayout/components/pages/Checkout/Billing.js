...
const Billing = ({ product,onNext }) => {
  const [bill, setBill] = useState(false);
  return (
    <div className="checkout__billing">
      <div className="checkout__part-info">
        <div className="content__title">Địa chỉ thanh toán của bạn là gì?</div>

        <Form.Item name="keep-login">
          <div className="login__option">
            <input type="checkbox" onChange={()=>setBill(!bill)} className="checkout__checkbox" value={bill}/>
      ...
      <div className="checkout__part-info">
        <button className="round-button" onClick={()=>onNext(2)}>Tiếp tục</button>
      </div>
    </div>
  );
};
