...
export const convertUtcToLocalTime = (utcTime, format = DATE_FORMAT_DISPLAY) => {
    try {
        if (utcTime) return moment(moment.utc(utcTime, DEFAUL_FORMAT).toDate()).format(format);
        return '';
    } catch (err) {
        return '';
    }
...
export const convertLocalTimeToUtc = (localTime, format) => {
    try {
        if (localTime) return moment(localTime, DEFAUL_FORMAT).utc().format(format);
        return '';
    } catch (err) {
        return '';
    }
...
export const convertStringToDateTimeString = (
    strFormDateTime,
    fromFormat = DATE_FORMAT_VALUE,
    toFormat = DATE_FORMAT_DISPLAY,
) => {
...
export const convertUtcToTimezone = (utcTime, format = DATE_FORMAT_DISPLAY) => {
    const utcOffset =  7;
    try {
        if (utcTime && format) return moment.utc(utcTime, DEFAUL_FORMAT).utcOffset(utcOffset).format(format);
        return '';
    } catch (err) {
        return '';
    }
...
export const convertTimezoneToUtc = (localTime, format) => {
    const utcOffset =  7;
    try {
        if (localTime && format) return moment.utc(localTime, DEFAUL_FORMAT).utcOffset(-utcOffset).format(format);
        return '';
    } catch (err) {
        return '';
    }
